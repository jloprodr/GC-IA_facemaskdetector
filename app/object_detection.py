...
class ObjectDetection(object):
    ...
    def postprocess(self, prediction_outputs):
        ...
        return [{'probability': round(float(selected_probs[i]), 8),
                 ...
                 'boundingBox': {
                     'xmin': round(float(selected_boxes[i][0]), 8),
                     'ymin': round(float(selected_boxes[i][1]), 8),
                     'xmax': round(float(selected_boxes[i][2])+float(selected_boxes[i][0]), 8),
                     'ymax': round(float(selected_boxes[i][3])+float(selected_boxes[i][1]), 8)
                 }
